...
class BuildBurger extends Component {
  ...
  clickLessHandler = (name) => {
    this.setState({
      ...
      ingredients: {
        ...this.state.ingredients,
        [name]:
          this.state.ingredients[name] !== 0
            ? this.state.ingredients[name] - 1
            : 0,
      },
    });
  };
  ...
  continueOrderNow = async () => {
    ...
    this.props.history.push({
      pathname: '/checkout',
      search: `?ingredientsId=${ingredientsId}`,
    });
    try {
      await firebase.database().ref(`ingredients/1/${ingredientsId}`).set({
